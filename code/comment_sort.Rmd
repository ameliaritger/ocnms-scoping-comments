...
comm_popular <- comm_sort %>%
  ...
  top_n(n = 3, wt = number_of_comments) %>% #only keep top N categories
  ungroup() %>%
...
#Make it a geom_image segment plot with human icons, for presentation
ggplot(comm_plot, aes(x=name, y=number_of_comments, fill=location)) + 
...
ggplot(data=comments_top, aes(label=word, size=n)) +
  ...
  scale_size_area(max_size=11) +
    scale_color_gradient2(low = "#0072B2", mid = "#661100", high = "#332288", na.value=NA, limits=c(5,70), midpoint = 40) + #force specific color range
  theme_void()
